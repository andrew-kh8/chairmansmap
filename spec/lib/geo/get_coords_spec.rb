...
RSpec.describe Geo::GetPlotCoords do
  ...
  context "when client returns success" do
    it "returns coords" do
      expect(Apis::Cadaster::Client).to receive(:plot_coords).with(cadaster_number).and_return(Dry::Monads::Success(coords))
  ...
  context "when client returns failure" do
    it "returns coords" do
      expect(Apis::Cadaster::Client).to receive(:plot_coords).with(cadaster_number).and_return(Dry::Monads::Failure("example error"))
